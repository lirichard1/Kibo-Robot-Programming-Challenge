...
public class OpenCVModel {
    ...
    public String inference(Mat mat) throws IOException {
        // Resize to 224x224
        ...
        Mat blob = blobFromImage(
            ...
            1 / 255.0, // scalefactor
            size, // size
            new Scalar(0.4561 * 255.0, 0.4561 * 255.0, 0.4561 * 255.0), // mean
            false, // swaprb
            false, //crop
            CvType.CV_32F
        );
        ...
        Mat normed = new Mat();
        Core.divide(blob, Scalar.all(0.2234), normed);
        ...
        try {
            ...
            InputStream is = context.getAssets().open("resnet.onnx");
            byte[] fileBytes = new byte[is.available()];
            is.read(fileBytes);
            MatOfByte matBytes = new MatOfByte(fileBytes);

            Net net = Dnn.readNetFromONNX(matBytes);
            Log.i("ROT", "Model loaded, setting input");
            net.setInput(normed);
